...
class GetAverageExecutionTimeControllerTest {
    ...
    void whenGettingAverageExecutionTimeByIdWithValidRequest_thenReturns200() throws Exception {
        ...
        when(getAverageExecutionTimeUC.processById(eq(testId), eq(AverageTimeEnum.HOURS))).thenReturn(mockAverage);
    ...
    void whenGettingAverageExecutionTimeByIdThrowsException_thenReturns500() throws Exception {
        UUID testId = UUID.randomUUID();
        when(getAverageExecutionTimeUC.processById(eq(testId), eq(AverageTimeEnum.SECONDS))).thenThrow(new RuntimeException("Test exception"));
